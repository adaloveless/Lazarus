...
log_info "Generating $CFG..."
{
    ...
    echo "# Release: $TAG"
    # The Lazarus sources use C-style operators (ide/checkcompileropts.pas:199 is
    # `s+=...`, the same line upstream). Every stock FPC install gets them from the
    # fpc.cfg that fpcmkcfg writes ("-Sgic"). lazbuild --build-ide passes no syntax
    # switches, so without this line a plain `lazbuild --build-ide=` on r27 died at
    # checkcompileropts.pas(199,6) "C styled assignment operators are turned off"
    # (measured 2026-09-23). Only the c is taken, because it only adds syntax: g and i
    # would change what existing code does (i turns on inlining for every routine
    # marked inline), and that is not the installer's call to make.
    echo "-Sc"
