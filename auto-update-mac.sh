...
# Generate a VP-tree-local fpc.cfg for $TARGET if one does not exist. The VP
# compiler has no baked-in unit search path on macOS (it relies on an fpc.cfg),
# and the existing hand-written ~/.fpc.cfg points at the installed release app,
# not the source tree. Sibling of build-release.sh's DARWIN_*_CFG.
ensure_mac_cfg() {
    if [ -f "$MAC_CFG" ]; then
        return 0
    fi
    ...
    if [ ! -d "$rtl_units" ]; then
        log_err "RTL units not built yet at $rtl_units"
        log_err "Build them first: make -C $VP_DIR rtl PP=$VP_COMPILER OPT=\"-n @$MAC_CFG\""
        return 1
    fi
...
-k-ld_classic
EOF
    log_ok "Generated $MAC_CFG (units: $rtl_units, SDK: $SDK_PATH)"
}
...
rebuild_vp_packages() {
    ...
    if [ ! -d "$rtl_units" ]; then
        ...
        make -C "$VP_DIR" rtl PP="$VP_COMPILER" OPT="-n @$MAC_CFG" 2>&1 | tail -3
    fi
...
if [ -z "$VP_COMPILER" ]; then
    log_err "Could not find a VibePascal compiler under $VP_DIR/compiler"
    log_err "Build it first: cd $VP_DIR && make compiler (then set VP_COMPILER to override)"
    exit 1
fi
