...
# From here to the end of the script everything runs inside this try, so the
# scratch dir is removed on EVERY way out, as install-lazarus.sh does with
# `trap 'rm -rf "$TMP_WORK"' EXIT`: a normal finish, each `exit 1` below (the one
# inside Download-File too), and any terminating error under
# $ErrorActionPreference = "Stop". Until 2026-09-23 it was removed on none of
# them. Measured that day on pwsh 7.6.6: a successful install left its
# lazarus-install-XXXXXXXX behind with the 244 MB tarball still in it, and so did
# a digest mismatch, a failed extraction and a thrown error; inside this try they
# all leave nothing and keep their exit code. Windows PowerShell 5.1 was not run.
# The try opens only once $tmp exists, so the finally is never handed an unset path.
try {
...
"@ -Encoding ASCII
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-ErrorX "Compiler smoke test failed -- compiler output:"
    ...
    # -LiteralPath, because %TEMP% can contain [ or ], which -Path reads as a
    # wildcard. Whatever cannot be removed (a file a virus scanner still holds open,
    # say) stays behind with a warning; the rest still goes, and the run keeps its
    # own exit code.
    Remove-Item -LiteralPath $tmp.FullName -Recurse -Force -ErrorAction SilentlyContinue
    if (Test-Path -LiteralPath $tmp.FullName) {
        Write-Warn "Could not remove all of $($tmp.FullName); delete it by hand."
    }
}
